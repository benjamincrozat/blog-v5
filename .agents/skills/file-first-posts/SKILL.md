...
2. If copy or scope changed, refresh internal links and the related-posts block when allowed.
3. Capture and upload needed images. For Discover/News-focused posts, the main image should be original when feasible and at least 1200 px wide.
4. Update `published_at` / `modified_at` when needed.
5. Run `php artisan app:sync-posts` or `php artisan app:generate-post-image <slug>`.
6. Only use browser validation when the article needs it.
7. Keep deploy behavior in mind: deployment should sync posts before sitemap generation.
